...
import com.rollerite.plugin.models.TPARequest;
...
public class TPAService {
    ...
    public TPAService(RolleritePlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean sendTPARequest(Player from, Player to) {
        ...
        if (isOnCooldown(fromId)) {
            ...
            plugin.getMessageService().sendMessage(from, "tpa-cooldown", "time", String.valueOf(remaining));
            return false;
        }
    ...
    public boolean acceptTPARequest(Player player) {
        ...
        if (request == null) {
            plugin.getMessageService().sendMessage(player, "tpa-none");
            return false;
        }
        
        Player from = request.getFrom();
        if (!from.isOnline()) {
            plugin.getMessageService().sendMessage(player, "tpa-expired");
        ...
        plugin.getMessageService().sendMessage(from, "tpa-accepted");
    ...
    public boolean denyTPARequest(Player player) {
        ...
        Player from = request.getFrom();
        if (from.isOnline()) {
            plugin.getMessageService().sendMessage(from, "tpa-denied");
        }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (request != null && request.getFrom().isOnline()) {
                    plugin.getMessageService().sendMessage(request.getFrom(), "tpa-expired");
                }
            }
